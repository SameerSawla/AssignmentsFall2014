...
{
  const std::vector<Point2D> & contained_points = points_contained();
  if(is_point_within(p))
  { 
    for(unsigned int i=0;i<contained_points.size();i++)
    {
      if(p.x()==contained_points[i].x() && p.y()==contained_points[i].y())
      {
        return false;
      }
      if(p.x()!=contained_points[i].x() && p.y()!=contained_points[i].y() && i==contained_points.size()-1)
      {
       setMPoints(p);
       return true;
      }
...
void Rectangle::setMPoints(const Point2D &p)
{
...
std::vector<Point2D> points_in_both(const Rectangle& r1, const Rectangle& r2) {
  ...
  {
    for(unsigned int j=0;i<r2_points.size();j++)
    {
      if((r1_points[i].x()==r2_points[j].x()) && (r1_points[i].y()==r2_points[j].y()))
      {
        temp.push_back(r1_points[i]);
      }
    }
  }
  return temp;
